    ...
    async def ban(self, ctx, user: discord.Member, reason):
        ...
        if getlang is None:
            ...
            ban.add_field(name=f'**User {user.name} has been banned by {ctx.author.name}**',
                          ...
                          inline=False)
            ban.add_field(name='If you want to set separate channel for notifications type click **Dashboard** below',
    ...
    async def kick(self, ctx, user: discord.Member, reason):
        ...
        if getlang is None:
            ...
            kick.add_field(name=f'**User {user.name} has been kicked by {ctx.author.name}**',
                          value='**__________________**',
                          inline=False)
            kick.add_field(name='If you want to set separate channel for notifications type click **Dashboard** below',
                          value='**__________________**', inline=False)
    ...
    async def clear(self, ctx, amount: int):
        if amount < 0:
            ...
            if amount > 300:
                ...
                getlang = Language.get_or_none(guild_id=ctx.guild.id)
                if getlang is None:
                    kick = discord.Embed(title='Messages deleted', colour=0xf1c40f)
                    kick.add_field(name=f'**User {ctx.author.name} has deleted {amount} messages in channel {ctx.channel}**',
                                   value='**__________________**',
                                   inline=False)
                    kick.add_field(
                    ...
                    for language in Language.select().where(Language.guild_id == ctx.guild.id):
                        if language.lang == 'en':
                            ...
                            kick = discord.Embed(title='Meldungen gelöscht', colour=0xf1c40f)
                            kick.add_field(name=f'**User {ctx.author.name} hat gelöscht {amount} Meldungen im Kanal {ctx.channel}**',
                                           value='**__________________**', inline=False)
                            kick.add_field(
                                name='Wenn Sie einen separaten Kanal für den Benachrichtigungstyp festlegen möchten, klicken Sie unten auf **Dashboard**',
                                value='**__________________**', inline=False)
