            ...
            if getlang is None:
                ...
                for language in Language.select().where(Language.guild_id == member.guild.id):
                    if language.lang == 'en':
                        ...
                        join = discord.Embed(title='User beitrat den Server', colour=0xf1c40f)
                        join.set_thumbnail(url=member.avatar)
                        join.add_field(name=f'**User {member.name} hat den Server beigetretet**',
                                      value='**__________________**', inline=False)
                        if israider == "True":
                            join.add_field(name='**Dieser User ist ein Raider!**',
                                           value='**__________________**',
                                           inline=False)
                            join.add_field(name='**Spezifizierte Aktion gegen Raiders:**',
                                           value=f'```{isspecified}```',
                                           inline=False)
                        else:
                            pass
